(function () {
    angular
        ...
        .controller('AdminFarmersController', function ($scope, $http, $state, $stateParams) {
            $scope.farmer_id = $stateParams.farmerId;
            $scope.farmer = _.find($scope.corporations, function (i) {
                return i.id == $scope.corporation_id;
            });

            // edit farmer
            $scope.saveFarmer = function (id, data) {
                angular.extend(data, {id: id});
                return $http.put('/api/farmers/' + id, data);
            };

            // remove farmer
            $scope.removeFarmer = function (index, id) {
                $http.delete('/api/farmers/' + id);
            ...
            $scope.addFarmer = function () {
                $scope.inserted = {
                    id: $scope.farmers.length + 1,
                    farmer: null,
                    path: null,
                    is_required: false
                };
                $scope.farmers.push($scope.inserted);
                $state.go("admin.farmers.edit", {farmerId: $scope.inserted.id});
            };
        })
        .controller('AdminCorporationsController', function ($scope, $http, $state, $stateParams) {
            $scope.corporation_id = $stateParams.corporationId;
            $scope.corporation = _.find($scope.corporations, function (i) {
                return i.id == $scope.corporation_id;
            });

            // edit corporation
            $scope.saveCorporation = function (id, data) {
                angular.extend(data, {id: id});
                return $http.put('/api/corporations/' + id, data);
            };

            // remove corporation
            $scope.removeCorporation = function (index, id) {
                $http.delete('/api/corporations/' + id);
                $scope.corporations.splice(index, 1);
            };
            ...
            $scope.addCorporation = function () {
                $scope.inserted = {
                    id: $scope.corporations.length + 1,
                    corporation: null,
                    path: null,
                    is_required: false
                };
                $scope.corporations.push($scope.inserted);
                $state.go("admin.corporations.edit", {corporationId: $scope.inserted.id});
            };
        })
        ...
        .controller('AdminRegionsController', function ($scope, $http, $state, $stateParams) {
            ...
            $scope.region = _.find($scope.corporations, function (i) {
                return i.id == $scope.corporation_id;
            });
