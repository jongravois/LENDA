(function () {
    ...
    function TermsController($scope, $state, $stateParams,
                             AppFactory, ConditionsFactory, ExceptionsFactory,
                             GlobalsFactory, LoansFactory, InitialData) {
        var procFee, svcFee;
        ...
        function calcTerms() {
            ...
            if (($scope.loan.loan_type_id === 2 || $scope.loan.loan_type_id === 6) && $scope.loan.fins.fee_processing_onTotal) {
                procFee = ($scope.loan.fins.fee_processing_percent / 100) * ($scope.loan.fins.commit_arm + $scope.loan.fins.commit_dist);
            } else {
                procFee = ($scope.loan.fins.fee_processing_percent / 100) * $scope.loan.fins.commit_arm;
            } // end if

            if (($scope.loan.loan_type_id === 2 || $scope.loan.loan_type_id === 6) && $scope.loan.fins.fee_service_onTotal) {
                svcFee = ($scope.loan.fins.fee_service_percent / 100) * ($scope.loan.fins.commit_arm + $scope.loan.fins.commit_dist);
            } else {
                svcFee = ($scope.loan.fins.fee_service_percent / 100) * $scope.loan.fins.commit_arm;
            } // end if
            ...
            var doTerms = {
                ...
                int_arm: (($scope.loan.loan_days / 365) * 0.5) * ($scope.loan.fins.int_percent_arm / 100) * $scope.loan.fins.commit_arm,
                int_percent_dist: $scope.loan.fins.int_percent_dist,
                int_dist: (($scope.loan.loan_days / 365) * 0.5) * ($scope.loan.fins.int_percent_dist / 100) * $scope.loan.fins.commit_dist
            };
            return doTerms;
        }
    } // end function
