(function () {
    ...
    function LoansProcessor($http, $q, API_URL, AppFactory) {
        ...
        function processFins(loan) {
            return {
                amount_requested: loan.financials.amount_requested,
                claims_percent: loan.financials.claims_percent,
                supplement_insurance_discount_percent: loan.financials.supplement_insurance_discount_percent,
                fsa_assignment_percent: loan.financials.fsa_assignment_percent,
                other_discount_percent: loan.financials.other_discount_percent,
                disc_prod_percent: loan.financials.disc_prod_percent,
                equipment_percent: loan.financials.equipment_percent,
                realestate_percent: loan.financials.realestate_percent,
                non_rp_percent: loan.financials.non_rp_percent,
                int_percent_arm: loan.financials.int_percent_arm,
                fee_processing_percent: loan.financials.fee_processing_percent,
                fee_processing_onTotal: !!loan.financials.fee_processing_onTotal,
                int_percent_dist: loan.financials.int_percent_dist,
                int_percent_other: loan.financials.int_percent_other,
                fee_service_percent: loan.financials.fee_service_percent,
                fee_service_onTotal: !!loan.financials.fee_service_onTotal,
                total_fee_percent: loan.financials.total_fee_percent,
                disc_ins_percent: loan.financials.disc_ins_percent,
                collateral_equipment: loan.financials.collateral_equipment,
                collateral_realestate: loan.financials.collateral_realestate,

                // Calculations below
                total_acres: AppFactory.getTotalAcres(loan.loancrops),
                total_claims: AppFactory.getTotalClaims(loan),
                total_fsa_payment: AppFactory.getTotalFSAPayment(loan),
                commit_arm: AppFactory.calcTotalCommitArm(loan),
                commit_dist: AppFactory.calcTotalCommitDist(loan),
                commit_other: AppFactory.calcTotalCommitOther(loan),
                commit_total: AppFactory.calcTotalCommitArm(loan),
                proc_fee: AppFactory.getFeeForArmProc(loan),
                proc_fee_arm_only: AppFactory.getFeeForArmProc(loan),
                srvc_fee: AppFactory.getFeeForArmSrvc(loan),
                srvc_fee_arm_only: AppFactory.getFeeForArmSrvc(loan),
                fee_total: AppFactory.getFeesForArm(loan),
                // COMMIT + FEES
                principal_arm: AppFactory.getArmPrincipal(loan),
                principal_dist: AppFactory.getDistPrincipal(loan),
                principal_other: AppFactory.getOtherPrincipal(loan),
                principal: AppFactory.getTotalPrincipal(loan),
                arm_and_dist: Number(AppFactory.getArmPrincipal(loan)) + Number(AppFactory.getDistPrincipal(loan)),
                int_arm: AppFactory.getArmInterest(loan),
                int_dist: AppFactory.getDistInterest(loan),
                int_other: 0,
                total_revenue: AppFactory.calcTotalRevenue(loan),
                //total_balance: AppFactory.calcArmTotalBalance(loan),
                //remaining_balance: AppFactory.calcArmRemainingBalance(loan),
                cash_flow: AppFactory.calcCashFlow(loan),
                //risk: AppFactory.calcRiskMargin(loan),
                //adj_risk: AppFactory.calcAdjustedRiskMargin(loan),
                //guaranty: AppFactory.calcInsuranceTotalGuarantee(loan),
                //prod: AppFactory.calcLoanProd(loan),
                //adj_prod: AppFactory.calcLoanAdjProd(loan),
                //disc_adj_prod: AppFactory.calcLoanDiscAdjProd(loan),
                //ins_disc_prod: AppFactory.calcLoanInsDiscProd(loan),
                //disc_ins: AppFactory.calcLoanDiscIns(loan),
                //collateral: AppFactory.calcTotalCollateral(loan)
            };
        }
